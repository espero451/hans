...
from hans.core.auth import User, require_admin
...
@router.get("/status")
async def get_dispatcher_status(current_user: User = Depends(require_admin)) -> dict:
    # Return current dispatcher status for the UI.
    return dispatcher_service.status()


@router.post("/restart")
async def restart_dispatcher(current_user: User = Depends(require_admin)) -> dict:
...
async def get_dispatcher_trace(current_user: User = Depends(require_admin)) -> str:
